...
// Constructor
template <typename T>
Matrix2D<T>::Matrix2D(int r, int c): n_rows_(3), n_cols_(3) {
	Initialisation(r, c);
}

template <typename T>
bool Matrix2D<T>::Initialisation(const int& r, const int& c) {
...
template <typename T>
void Matrix2D<T>::SetDisplayWidth(const int& pcs) {
	int n_intd = 1;
	int max_n_intd = 1;
...
template <typename T>
std::vector<T>& Matrix2D<T>::operator[] (const int& index) {
	return main_matrix_[index];
}

template <typename T>
void Matrix2D<T>::AssignElement(const int& r, const int& c, const T& val) {
...
template <typename T>
static Matrix2D<T>& Matrix2D<T>::LUComposition(const std::vector< std::vector<double> >& m_lu) {
	if (n_rows_ != n_cols_) return NULL;
	Matrix2D<T> ans(n_rows_);
...
template <typename T>
void Matrix2D<T>::DisplayLU(int pcs) {
	std::vector< std::vector<double> >& mlu = matrix_lu_;
	std::vector<int>& pr = matrix_pr_;
	std::vector<int>& pc = matrix_pc_;
...
Matrix2D<T> Matrix2D<T>::operator*(const Matrix2D &m) {
	...
	Matrix2D<T> ans(n_rows_, m.n_cols_);
	for (int i = 0; i < n_rows_; i++) {
